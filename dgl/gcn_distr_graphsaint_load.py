...
from torch_geometric.datasets import Planetoid, PPI, Reddit2,Flickr
...
def start_time(group, rank, subset=False, src=None):
    ...
    if not run in barrier_time.keys():
        barrier_time[run] = {}
    if not rank in barrier_time[run].keys():
        barrier_time[run][rank] = 0 
...
#        elif i == size - 1:            inputs_recv = torch.cuda.FloatTensor(am_partitions[i].size(1), inputs.size(1), device=device).fill_(0)
        ...
        if horizontal_tiled:
            ...
            dur = stop_time(group, rank, tstart_comm)
            if not run in comm_time.keys():
                comm_time[run] = {}
            if not rank in comm_time[run].keys():
                comm_time[run][rank] = 0 
            if not run in bcast_comm_time.keys():
                bcast_comm_time[run] = {}
            if not rank in bcast_comm_time[run].keys():
                bcast_comm_time[run][rank] = 0 
            comm_time[run][rank] += dur
        ...
        if not run in comp_time.keys():
            comp_time[run] = {}
        if not rank in comp_time[run].keys():
            comp_time[run][rank] = 0 
        if not run in scomp_time.keys():
            scomp_time[run] = {}
        if not rank in scomp_time[run].keys():
            scomp_time[run][rank] = 0 
        comp_time[run][rank] += dur
...
class GCNFunc(torch.autograd.Function):
    @staticmethod
    def forward(ctx, inputs, weight, adj_matrix, am_partitions, rank, size, group, func, test=False):
        global comm_time
        global comp_time
        global dcomp_time
        global run
        ...
        if ht:
            ...
            if not test:
                ...
                if not rank in comp_time[run].keys():
                    comp_time[run][rank] = 0 
                if not run in dcomp_time.keys():
                    dcomp_time[run] = {}
                if not rank in dcomp_time[run].keys():
                    dcomp_time[run][rank] = 0 
                comp_time[run][rank] += dur
                dcomp_time[run][rank] += dur
            z = broad_func(adj_matrix.size(0), am_partitions, z, rank, size, group, ht)
            #print('x after spmm:',z)
        ...
        if activations:
            if func is F.log_softmax:
                h = func(z, dim=1)
                #print('x after softmax:',h)
            elif func is F.relu:
                h = func(z)
                #print('x after relu:', h)
            else:
                h = z
...
def train(inputs, weight1, weight2, adj_matrix, am_partitions, optimizer, data, rank, size, group, horizontal_tiled, labels, row_count):
    ...
    #print('3')
    #print('*'*30)
    #print('Input feat:', inputs)
    #print('Input adj:', adj_matrix)
    #print('Input adj_part:', am_partitions)
    outputs = GCNFunc.apply(inputs, weight1, adj_matrix, am_partitions, rank, size, group, F.relu)
    #print('4')

    #print('*'*30)
    #print('output 1st layer:', outputs)
    ...
    # if datay_rank.size(0) > 0:
        ...
        #print('label:', datay_rank.shape)
    
        loss = F.nll_loss(outputs, datay_rank.long())
...
def test(outputs, data, vertex_count, rank):
    ...
    for _, mask in data('train_mask', 'val_mask', 'test_mask'):
        ...
        if graphname == 'products':
            y = data.y[mask.bool()]#.to(rank)#.squeeze(dim=1)
            #y = data.y[mask.bool()].to(rank).squeeze(dim=1)
        else:
            y = data.y[mask.bool()]#.to(rank)#.squeeze(dim=1)
        ...
        #print(pred.eq(y).cpu().sum()/mask.cpu().sum())
        #exit()
        acc = pred.cpu().eq(y.cpu()).sum().item() / mask.cpu().sum().item()
        #acc = pred.eq(y[mask.bool()]).sum().item() / mask.sum().item()
        accs.append(acc)

    return accs

def oned_partition_old(rank, size, inputs, adj_matrix, data, features, classes, device, edge_w=None):
    global row_count
    global col_count
    #print('inputs:', inputs)
...
def full_test(inputs, weight1, weight2, adj_matrix, am_partitions, data, rank, size,
    ...
    if not normalization:
        ...
        edge_w = None
    #adj_matrix, edge_w = gcn_norm(adj_matrix, num_nodes=len(data.x))
    #print(inputs)
    #exit()
    #if isinstance(inputs, np.array):
    #inputs = torch.from_numpy(inputs)
    #print(adj_matrix)
    #print('Device for testing:', device)
    ...
    outputs = GCNFunc.apply(inputs, weight1, adj_matrix, am_partitions, rank, size, group, F.relu)
    #outputs = GCNFunc.apply(inputs, weight1, adj_matrix, am_partitions, rank, size, group, F.relu, True)
    #outputs = F.dropout(outputs, 0.5, training=True)
    #print('6')
    outputs = GCNFunc.apply(outputs, weight2, adj_matrix, am_partitions, rank, size, group, F.log_softmax)
    #outputs = GCNFunc.apply(outputs, weight2, adj_matrix, am_partitions, rank, size, group, F.log_softmax, True)
    #print('7')
    #if rankog==0:
...
def run(rank, size, inputs, graphs, data, features, classes, device, orig_adj=None, group=None):
    ...
    for (nodes_subg, subg, inputs_subg, labels, edgew_subg) in tqdm(graphs):
        # nodes_subg <- subg[sub_nodeid]
        #sug = symmetric(subg)
#        inputs_subg = inputs[nodes_subg.long()]
        #print('Input subg:', subg)
        #inputs_subg = torch.from_numpy(inputs_subg)#.double()
        #print('Input feat:', inputs_subg)
        #print('Edge w:', edge_w)
        inputs_loc, adj_matrix_loc, am_pbyp, horizontal_tiled, rowcount, colcount = oned_partition(rank, size, inputs_subg, subg, data,
                                                                    features, classes, device, edge_w=edgew_subg)

        print('moving to device:', device)
        labels_subg = labels.to(device)#data.y[nodes_subg.long()]
        inputs_loc = inputs_loc.float().to(device)
        #print('inputsloc:', inputs_loc.dtype)
        adj_matrix_loc = adj_matrix_loc.to(device)
        for i in range(len(am_pbyp)):            
            am_pbyp[i] = am_pbyp[i].t().coalesce().to(device)
        #am_pbyp = [am_pbyp[0].t().coalesce().to(device)]
        train_loader.append((inputs_loc, adj_matrix_loc, am_pbyp, horizontal_tiled,  labels_subg, rowcount, colcount))
        #print(inputs.shape)
        #print(inputs_subg.shape)
        #print(inputs_loc.shape)
        #print('Inputs subg:', inputs_subg)
        #print('Inputs loc:', inputs_loc)
        #exit()
...
#    exit()
    ...
    for i in range(run_count):
        run = i
        torch.manual_seed(0)
        if graphname in ['arxiv', 'products']:
            g = 'ogbn-' + graphname
        else:
            g = graphname
        if args.load:
            weight1_nonleaf = torch.load(f'dgl_gs_{g}_w1.pt')
            weight1_nonleaf.requires_grad=True
        else:
            weight1_nonleaf = torch.rand(features, mid_layer, requires_grad=True)
        #weight1_nonleaf = torch.load('gs_w1.pt')
        #f'dgl_gs_{args.dataset}_w1.pt'
        print(weight1_nonleaf.shape)
        print(weight1_nonleaf)
        weight1_nonleaf = weight1_nonleaf.to(device)
        weight1_nonleaf.retain_grad()

        if args.load:
            weight2_nonleaf = torch.load(f'dgl_gs_{g}_w2.pt')
            weight2_nonleaf.requires_grad=True
        else:
            weight2_nonleaf = torch.rand(mid_layer, classes, requires_grad=True)
        #weight2_nonleaf = torch.load('gs_w2.pt')
        print(weight2_nonleaf.shape)
        print(weight2_nonleaf)
        ...
        #exit()
        LR = args.lr 
        optimizer = torch.optim.Adam([weight1, weight2], lr=LR)
        dist.barrier(group)
        ...
        for epoch in range(500):
            ...
            #print('1')
            for i, (inputs_loc, adj_matrix_loc, am_pbyp, horizontal_tiled,  labels_subg, rowcount, colcount) in enumerate(train_loader):
                #print('2')
                ep_s = time.time()
                row_count = rowcount
                col_count = colcount
            ...
            if epoch % eval_freq == 0:
                ...
                if rank == 0:
                    ...
                    print(logline)
                    logline = f'{graphname},RDM-{LR}-{ws}-ns,{args.topo},{epoch:02d},{cumulative_time:.4f},{tmp_test_acc:.4f}\n'
...
def main():
    ...
    if not download:
        ...
        #    for i in range(8):
        #        a = torch.zeros(1)
        #        a = a.to(torch.device('cuda:{}'.format(i)))
        #        print('cuda:{} OK'.format(i))
    ...
    #    print(edge_index)
       ...
       #     (node_ids, adj_subg, edge_w, norm_loss) = minibatch.one_batch(mode='train')
       ...
       #     subgraphs.append((node_ids, adj_subg, edge_w, norm_loss))
       # if rank==0:
       #     for i, (node_ids, adj_subg, edge_w, norm_loss) in enumerate(subgraphs):
       #         torch.save(node_ids, 'subgs-reddit/nodeids_{}.pt'.format(i))
       #         torch.save(adj_subg, 'subgs-reddit/adj_subg_{}.pt'.format(i))
       #         inputs_subg = inputs[node_ids.long()]
       #         torch.save(inputs_subg, 'subgs-reddit/inputs_subg_{}.pt'.format(i))
       #         torch.save(edge_w, 'subgs-reddit/edge_w_{}.pt'.format(i))
       ...
       #     #print(adj_subg.shape)
            adj_full_norm = data.edge_index
            data.edge_index = edge_index
        elif 'products' in graphname or 'arxiv' in graphname:
            path = '/scratch/general/nfs1/u1320844/dataset'
            dataset = PygNodePropPredDataset('ogbn-{}'.format(graphname), path,transform=T.NormalizeFeatures())
            ...
            if 'arxiv' in graphname:
                print('making adj symmetric')
                edge_index = symmetric(data.edge_index)
                data.edge_index = edge_index
                _, data.edge_weight = gcn_norm(data.edge_index, num_nodes=num_nodes)
            adj_full_norm = data.edge_index
            train_mask = torch.zeros(num_nodes)
            ...
            num_classes = dataset.num_classes
            edge_index, data.edge_weight = gcn_norm(data.edge_index, num_nodes=num_nodes)
            adj_full_norm = data.edge_index
            data.edge_index = edge_index
        print('loading subgs')
        subgraphs = []
        #pref = '/uufs/chpc.utah.edu/common/home/u1320844/OGB_models/ogb/examples/nodeproppred/reddit/prod_subgs/'
        ...
        #pref = '/uufs/chpc.utah.edu/common/home/u1320844/OGB_models/ogb/examples/nodeproppred/new/asplos/pyg/{}_subgs/'.format(gg.lower())
        pref = '/scratch/general/nfs1/u1320844/dataset/asplos/{}_subgs/'.format(gg.lower())
        for i in range(32):
                node_ids = torch.load(pref+'train_mask_{}.pt'.format(i))
                adj_subg = torch.load(pref+'adj_{}.pt'.format(i))
                # TODO add self loop
                if args.topo == 'lower':
                    adj_subg = torch.stack((adj_subg[1], adj_subg[0]))
                    adj_subg, _ = add_remaining_self_loops(adj_subg) 
                elif args.topo == 'sym':
                    adj_subg = symmetric(adj_subg)
                else: # upper
                    adj_subg, _ = add_remaining_self_loops(adj_subg) 
                inputs_subg = torch.load(pref+'x_{}.pt'.format(i))
                label = torch.load(pref+'y_{}.pt'.format(i))
                # TODO add edge_w
                if not normalization:
                    _, edgew_subg = gcn_norm(adj_subg, num_nodes=num_nodes)
                else:
                    edgew_subg = torch.ones(adj_subg[0].shape) 

                #edgew_subg = gcn_norm(adj_subg, num_nodes=node_ids.sum())
                #if graphname == 'products':
                #    label = torch.squeeze(label, dim=1)
                #edge_w = torch.load('subgs-reddit/edge_w_{}.pt'.format(i))
                #norm_loss = torch.load('subgs-reddit/norm_loss_{}.pt'.format(i))
                #subgraphs.append((node_ids, adj_subg, inputs_subg, edge_w, norm_loss))
                subgraphs.append((node_ids, adj_subg, inputs_subg, label, edgew_subg))
...
if __name__ == '__main__':
    ...
    parser.add_argument("--acc_csv", type=str, default='test.csv')
