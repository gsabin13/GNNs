...
from torch_geometric.datasets import Planetoid, PPI, Reddit2
...
def main():
    ...
    #    print(edge_index)
       ...
       #     #print(adj_subg.shape)
        ...
        if 'ducts' in graphname:
            path = '/scratch/general/nfs1/u1320844/dataset'
            dataset = PygNodePropPredDataset('ogbn-products', path,transform=T.NormalizeFeatures())
            ...
            pref = '/uufs/chpc.utah.edu/common/home/u1320844/OGB_models/ogb/examples/nodeproppred/reddit/oral/full/'
            edge_index = torch.load(pref+'adj_full.pt')
            adj_full_norm = edge_index
            x = torch.load(pref+'x_full.pt')
            y = torch.load(pref+'y_full.pt')
            train_mask = torch.load(pref+'train_mask_full.pt')
            val_mask = torch.load(pref+'val_mask_full.pt')
            test_mask = torch.load(pref+'test_mask_full.pt')
            data = Data()
            data.train_mask = train_mask
            data.val_mask = val_mask
            data.test_mask = test_mask
            ...
            num_classes = 25 
            data.edge_index, data.edge_weight = gcn_norm(data.edge_index, num_nodes=num_nodes)
            data.edge_weight = torch.ones(data.edge_index.shape[1])
            print('Edgew:', data.edge_weight)
            #exit()
        else:
        ...
        pref = '/uufs/chpc.utah.edu/common/home/u1320844/OGB_models/ogb/examples/nodeproppred/reddit/oral/subgs/'
        for i in range(32):
                node_ids = torch.load(pref+'train_mask{}.pt'.format(i))
                ...
                #edgew_subg = torch.ones(edgew_subg.shape)
                print('edgew:', edgew_subg)
