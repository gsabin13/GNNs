...
def main():
    ...
    #    print(edge_index)
    elif graphname in ['Flickr', 'reddit', 'products']:
       ...
       #     #print(adj_subg.shape)
        ...
        for i in range(30):
                node_ids = torch.load(pref+'mask{}.pt'.format(i))
                adj_subg = torch.load(pref+'adj{}.pt'.format(i))
                inputs_subg = torch.load(pref+'feat{}.pt'.format(i))
                label = torch.load(pref+'label{}.pt'.format(i))
                # TODO add edge_w
