...
class DistGCN(nn.Module):
    ...
    def init(self, in_feats, n_hidden, n_classes, n_layers, activation, dropout):
        ...
        if n_layers > 1:
            ...
            self.layers.append(GraphConv(in_feats, n_classes))
    ...
    def inference(self, g, x, batch_size, device):
        """
        Inference on full neighbors (i.e. without neighbor sampling).
        g : the entire graph.
...
def run(args, device, data):
    ...
    epoch = 0
    cumulative_time = 0
    if args.graph_name == 'reddit':
        ep_lim = 21
    elif 'ogb' in args.graph_name :
        ep_lim = 31

    for epoch in range(ep_lim):
        tic = time.time()
        accs = []
        #if cumulative_time>150:
        #    break
        sample_time = 0
        forward_time = 0
        backward_time = 0
        ...
        if epoch % 1 == 0:
            ...
            #tacc = th.Tensor(test_acc)
            tacc = test_acc.to(device)#th.tensor(4)
            dist.all_reduce(tacc, dist.ReduceOp.SUM)
            tacc = th.mean(tacc) / 4
