...
def test(outputs, data, vertex_count, rank):
    ...
    for _, mask in data('train_mask', 'val_mask', 'test_mask'):
        ...
        #exit()
        acc = pred.eq(y).sum().item() / mask.sum().item()
...
def run(rank, size, inputs, graphs, data, features, classes, device, orig_adj=None, group=None):
    ...
    for (nodes_subg, subg, inputs_subg, labels) in tqdm(graphs):
...
def main():
    ...
    #    print(edge_index)
       ...
       #     #print(adj_subg.shape)
        ...
        #pref = '/uufs/chpc.utah.edu/common/home/u1320844/OGB_models/ogb/examples/nodeproppred/reddit/subgs/'
        for i in range(30):
                ...
                subgraphs.append((node_ids, adj_subg, inputs_subg, label))
