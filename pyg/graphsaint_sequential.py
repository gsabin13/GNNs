...
#from graphsaint_ddp import GCN
...
if args.save:
    ...
    if not osp.exists(dir_):
        os.mkdir(dir_)
    torch.save(data.edge_index, '{}/adj_full.pt'.format(dir_))
    torch.save(data.x, '{}/x_full.pt'.format(dir_))
    torch.save(data.y, '{}/y_full.pt'.format(dir_))
    torch.save(data.train_mask, '{}/train_mask_full.pt'.format(dir_))
    torch.save(data.val_mask, '{}/val_mask_full.pt'.format(dir_))
    torch.save(data.test_mask, '{}/test_mask_full.pt'.format(dir_))
#exit()
if args.load:
    loader = []
...
model = GCN(data.x.size(-1),128,dataset.num_classes,2,0.2).to(device)
...
#    model.set_aggr('add' if args.use_normalization else 'mean')
    ...
    for i, data in enumerate(loader):
        if args.save:
            ...
            if not osp.exists(dir_):
                os.mkdir(dir_)
            torch.save(data.edge_index, '{}/adj_{}.pt'.format(dir_, i))
            torch.save(data.x, '{}/x_{}.pt'.format(dir_, i))
            torch.save(data.y, '{}/y_{}.pt'.format(dir_, i))
            torch.save(data.train_mask, '{}/train_mask_{}.pt'.format(dir_, i))
            #torch.save(data.node_norm, '{}/node_norm_{}.pt'.format(dir_, i))
            torch.save(data.edge_weight, '{}/edge_weight_{}.pt'.format(dir_, i))
            #torch.save(data.edge_norm, '{}/edge_norm_{}.pt'.format(dir_, i))
        else:
...
for epoch in range(50):
    ...
    accs = test(args, data)
    logline = f'{args.dataset},Sequential,{epoch:02d},{total_time:.4f},{loss:.4f},{accs[0]:.4f},{accs[1]:.4f},{accs[2]:.4f}\n'
    print(logline)
