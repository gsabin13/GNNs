...
ap.add_argument("--local_rank", type=int, default=30)
ap.add_argument("--world_size", type=int, default=30)
...
fileHandler = logging.FileHandler(output_dir+"/"+"{}_{}gpu_{}_{}_{}.log".format(data_name, ws,loader_type, bs, n_subgs))
...
if loader_type == 'c':
    ...
    l = GraphSAINTRandomWalkSampler(data,
                                         batch_size=bs,
                                         walk_length=3,
                                         num_steps=n_subgs,
                                         sample_coverage=0,
...
logger.info("Running GNN on: "+str(rank))
model = Net().to(rank)
...
for epoch in range(1, 1000):
    ...
    if rank == 0:
        train_acc, val_acc, tmp_test_acc = test(data, device_id)
        if val_acc > best_val_acc:
            best_val_acc = val_acc
            test_acc = tmp_test_acc
        log = '{},{},{},{:.4f},{:.4f},{:.4f},{:.4f}'
        logger.info(log.format(data_name, ws,epoch, total_time, train_acc, best_val_acc, test_acc))
elapsed_time = time.time() - start_time
